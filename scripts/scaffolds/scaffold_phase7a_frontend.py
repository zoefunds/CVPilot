"""
Phase 7A frontend: types, api client extension, RewritePanel, evaluation page wire-in.
"""
...
export function RewritePanel({ application }: Props) {
  ...
  // Poll active rewrite until terminal.
  useEffect(() => {
    if (!activeId) return;
...
# Wire the panel into the evaluation page.
# We do a precise insertion to avoid rewriting the entire detail file.
